...
Run it on save as a hook when in HTML mode, why not.

#+begin_src emacs-lisp
  (defun recker/html-mode-hook ()
    (add-hook 'before-save-hook #'recker/add-p-tags-to-buffer 0 t))

  (add-hook 'mhtml-mode-hook #'recker/html-mode-hook)
#+end_src
