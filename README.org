...
Add a capture template for ideas.

#+BEGIN_SRC emacs-lisp
  (let* ((opsat (recker/path 'docs "opsat.org"))
	 (dest `(file+headline ,opsat "Ideas"))
	 (template `("i" "idea" plain ,dest "%?" :empty-lines 1)))
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun recker/docs-sync ()
    ...
    (save-window-excursion
      (async-shell-command (format "git-sync.sh -d %s" (recker/path 'docs)) nil nil)))
#+END_SRC
