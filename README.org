...
** Ruby
   ...
   These are very much a work in progress.  As of now, I have to
   manually trigger rbenv to activate the right version with =M-x=
   =rbenv-use-corresponding=.  I'm not sure if that's correct or not.
