...
#+begin_src emacs-lisp
  (use-package flycheck
    ...
    ;; emacs-lisp-mode-hook also fires for lisp-interaction-mode (i.e. *scratch*),
    ;; since the latter is derived from the former, so guard against it here.
...
#+begin_src emacs-lisp
  (use-package copilot-chat
    ...
    :config (setq copilot-chat-org-prompt
                  (with-temp-buffer
                    (insert-file-contents-literally (concat user-emacs-directory "prompts/chef.txt"))
                    (buffer-string)))
...
Claude Code AI slop, via [[https://github.com/xenodium/agent-shell][agent-shell]], which talks to Claude over ACP and renders natively instead of emulating a terminal.  Before this works, install its external dependencies:
...
#+begin_src emacs-lisp
  (use-package agent-shell
    :ensure t
    :bind ("C-c v" . agent-shell-anthropic-start-claude-code)
    :config
    (keymap-set agent-shell-mode-map "C-c C-q" #'agent-shell-prompt-queue)
    (with-eval-after-load 'agent-shell-completion
      (defun recker/agent-shell-use-company (orig-fn)
        "Trigger company instead of the default completion UI, when available."
        (if (bound-and-true-p company-mode)
            (when (memq (char-before) '(?@ ?/))
              (company-manual-begin))
          (funcall orig-fn)))
      (advice-add 'agent-shell--trigger-completion-at-point :around #'recker/agent-shell-use-company)))
...
#+begin_src emacs-lisp
  (defun recker/python-mode-hook ()
    ;; disable flymake bullshit (native python mode always wants to turn it on)
    (flymake-mode -1))
...
#+begin_src emacs-lisp
  ...
  ;; Archive function for Gnus
  (defun recker/gnus-summary-archive ()
    "Archive the current message to Gmail's All Mail folder."
    (interactive)
    (let* ((group (gnus-group-real-name gnus-newsgroup-name))
           (method (gnus-find-method-for-group gnus-newsgroup-name))
           (server (nth 1 method))
           (archive-group (format "nnimap+%s:[Gmail]/All Mail" server)))
      (gnus-summary-move-article nil archive-group)))

  ;; Bind 'a' to archive in summary mode
  (add-hook 'gnus-summary-mode-hook
            (lambda ()
              (local-set-key (kbd "a") 'recker/gnus-summary-archive)))
#+end_src
...
3. Archive mail by pressing =a= to move it to the All Mail folder
...
#+begin_src emacs-lisp
  ...
  (setq org-capture-templates
        '(("t" "task" entry (file+olp+datetree "work.org") "* TODO %?\nSCHEDULED: %t")
          ...
          ("s" "standup" entry (file+olp+datetree "work.org") "* TODO Stand-up\nSCHEDULED: %t\n** Parking Lot [0/1]\n- [ ] %?")
          ("h" "habit" entry (file "daily.org")
           "* TODO %?\nSCHEDULED: <%<%Y-%m-%d %a> .+1d>\n:PROPERTIES:\n:STYLE: habit\n:REPEAT_TO_STATE: TODO\n:END:")
          ("e" "emotions" plain (file+olp+datetree "emotions.org")
           "- Joy: %^{Joy}\n- Anger: %^{Anger}\n- Sad: %^{Sad}\n- Fear: %^{Fear}\n- Disgust: %^{Disgust}\n- Behaviors: %^{Behaviors}\n" :immediate-finish t :jump-to-captured t)
          ("j" "journal" plain (file+olp+datetree "journal.org.gpg") "%?")
          ("b" "blog" plain (function recker/blog-target) (function recker/blog-template) :immediate-finish t :jump-to-captured t)))

  (defun recker/trigger-emotion-capture ()
    "Wait for the habit to roll over, then fire the capture."
    (when (and (string= org-state "DONE")
               (string-match-p "track emotions" (org-get-heading t t)))
      (run-at-time "1 sec" nil (lambda () (org-capture nil "e")))))

  (add-hook 'org-after-todo-state-change-hook #'recker/trigger-emotion-capture)
               ...
               '("h" "Habits"
                 ((agenda ""
                          ((org-agenda-span 'day)
                           ...
                           (org-agenda-overriding-header "Daily Habit Tracker"))))))
...
And sometimes the random functions get promoted to tools!

#+begin_src emacs-lisp
  (use-package simple-timer
    :vc (:url "https://github.com/arecker/simple-timer.el")
    :config (setq simple-timer-sound-file "~/.emacs.d/sounds/gong.wav"))
#+end_src
