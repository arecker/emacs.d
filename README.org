...
#+begin_src emacs-lisp
  ...
  (use-package claude-code
    ...
    :init
    (unless (file-directory-p (expand-file-name "elpa/claude-code" user-emacs-directory))
  ...
  (use-package claude-code-extras
    :ensure nil
    :load-path "~/.emacs.d/elpa/claude-code-emacs"
    :after claude-code
    :init
    (unless (file-directory-p (expand-file-name "elpa/claude-code-emacs" user-emacs-directory))
      (package-vc-install "https://github.com/lsy83971/claude-code-emacs"))
    :config
    (require 'claude-code-manager)
    (define-key claude-code-command-map (kbd "L") #'claude-code-manager)
    (remove-hook 'claude-code-start-hook #'claude-code--auto-open-input))
#+end_src
