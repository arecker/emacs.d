...
#+BEGIN_SRC emacs-lisp
  ...
  (defun recker/gfm-mode-hook ()
    (auto-fill-mode 0))

  (add-hook 'gfm-mode-hook 'recker/gfm-mode-hook)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  already captured in wunderlist (by title) is added."
    ...
    (let* ((headers `(("Content-Type" . "application/json")
	   ...
	   (these-items   (if (member (first (org-element-at-point)) '(plain-list item))
	   ...
	   (new-items (remove-if #'(lambda (i) (member (format "%s" i) current-items)) these-items)))
...
#+BEGIN_SRC emacs-lisp
  ...
  (global-set-key (kbd "C-c g") 'recker/send-list-at-point-to-wunderlist)
